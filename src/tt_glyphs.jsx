...
import { REF_SPRITES } from './tt_ref_sprites.js';
...
/* Point rendering for the Task Tracker, using the skin pack's icons EXACTLY.
   ...
   Three renderers share the same sprites:
   - paintRefStack(): canvas, for the Leaflet satellite map (sprites drawn
     at device-pixel size, rotation baked into a cached canvas)
   - <StackSvg/>: the same sprites as SVG <image>s (plan view, 3D overlay);
     flags / dots / delete markers still come from <GlyphDefs/>
...
/* sprite readiness: the map renderer subscribes with onIconsReady() and repaints once the images land */
...
/* ---------- reference skin: raster sprites cut from the concept mock ----------
   The product skin everywhere (web, phone, Android). Sprites come from
   src/tt_ref_sprites.js (generated by scripts/extract_ref_sprites.py). Sizes
   are the mock's own pixel sizes and scale with the row pitch:
   k = pitchPx / 36 (the mock's pitch). */
...
export function Glyph({ code, size = 20, style }) {
  if (REF_SPRITES[code]) return <RefSprite code={code} size={size} rotate={code === 's3' ? 66 : code === 's4' ? 90 : 0} style={style} />;
  return (
...
/* Plan-view / 3D-overlay points in SVG (plan units): the mock's sprites as
