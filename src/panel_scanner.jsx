        ...
        {tree.projects.map((p) => {
          ...
          return (
            <div key={p.id} style={{ ...card, borderLeft: "5px solid " + p.color }} onClick={() => { setProjId(p.id); setSecId(null); setRowId(null) }}>
              ...
              <div onClick={(e) => { e.stopPropagation(); deleteProject(p) }} style={{ ...NB, fontSize: 12, color: "#c00", marginTop: 10, letterSpacing: 1, textTransform: "uppercase" }}>Delete</div>
            </div>
          )
        })}
        ...
        {capture && (
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1, minWidth: m ? "100%" : 220 }}>
              ...
              <input type="number" inputMode="numeric" value={capture.panel} onChange={(e) => setCapture({ ...capture, panel: e.target.value })} style={{ ...IST, marginBottom: 14, width: 130 }} />
...
const APPS_SCRIPT = `function doPost(e){
  ...
  if (sh.getLastRow() === 0) sh.appendRow(['Timestamp','Serial','Project','Section','Row','Panel','By','Status','Note','Mode','ID']);
    ...
    if (n > 0) {
      ...
      for (var i = 0; i < ids.length; i++) {
        if (String(ids[i][0]) === String(d.id)) {
          ...
          else sh.getRange(i+2,1,1,11).setValues([[d.timestamp,d.serial,d.project,d.section,d.row,d.panel,d.by,d.status,d.note,'update',d.id]]);
